<?php
$connect = mysqli_connect($hostname= 'localhost', $username = 'root', $password ='', $database ='kurs');
...
<html>
    ...
    <body>
        ...
        <fieldset class="rate">
            ...
            и вуза в целом. Рассчеты производились по специальной формуле: avgR = avgR1 + avgR2 + avgR3 +
            ...
            значение рассчитывалось из следующих показателей: avgM, avgOS и avgD, которые значат среднее
                арифметическое значение оценок, среднее ариметическое значение зачётов и среднее арифметическое значение
                балла дипломов выпускников соответственно.
            </p>
        </fieldset>
            <div class = "bd">
                <table>
                    ...
                    <?php
                    $marks_fasiu = mysqli_query($connect,"SELECT * FROM `marks_fasiu`");
                    $marks_fasiu = mysqli_fetch_all($marks_fasiu);
                    foreach ($marks_fasiu as $marks_fasiu){
                        $avgM = round(($marks_fasiu[0] * 5 + $marks_fasiu[1] * 4 + $marks_fasiu[2] * 3 + $marks_fasiu[3] * 2)/$marks_fasiu[6], 1);
                        $avgOS = round((2422 * 5)/2458,1);
                        $avgR1 = round(($avgM + $avgOS + $marks_fasiu[5])/3,1);
                        echo '
                            <tr>
                                <td>Значения</td>
                                <td>'. $avgM .'</td>
            ...
            <div class="bdl">
                <table>
                    ...
                    $marks_rtf = mysqli_fetch_all($marks_rtf);
                    foreach ($marks_rtf as $marks_rtf){
                        $avgR = round(($avgR1 + $avgR2 + $avgR3 + $avgR4 + $avgR5 + $avgR6)/6,1);
                        echo '
                            <tr>
                                <td>Значения</td>
                                <td>'. $avgR .'</td>
